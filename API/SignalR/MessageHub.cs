...
namespace API.SignalR
{
    public class MessageHub : Hub
    {
        ...
        {
            // uisng SignalR Grouping, so need a group name
            ...
            var messages = await _unitOfWork.MessageRepository.GetMessageThread(Context.User.GetUsername(), otherUser).ConfigureAwait(false);

            if (_unitOfWork.HasChanges()) await _unitOfWork.Complete();

            await Clients.Caller.SendAsync("ReceiveMessageThread", messages).ConfigureAwait(false);
        }

        public override async Task OnDisconnectedAsync(Exception ex)
        {
            var group = await _removeFromMessageGroup().ConfigureAwait(false);
        ...
        public async Task SendMessage(CreateMessageDto createMessageDto)
        {
            ...
            {
                var connections = await _tracker.GetConnectionsForUser(recipient.UserName).ConfigureAwait(false);
                if (connections != null)
                {
        ...
        private string _getGroupName(string caller, string other)
        {
            ...
            return stringCompare ? $"{caller}-{other}" : $"{other}-{caller}"; // alphabetical
        }
    }
}
